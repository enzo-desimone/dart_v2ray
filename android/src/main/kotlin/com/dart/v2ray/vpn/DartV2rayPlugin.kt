...
import com.dart.v2ray.vpn.xray.core.XrayCoreManager
 ...
 */
class DartV2rayPlugin : FlutterPlugin, ActivityAware, PluginRegistry.ActivityResultListener, MethodChannel.MethodCallHandler {
    ...
    private var startMonitorReceiver: BroadcastReceiver? = null
    ...
    private fun handleStartVless(call: MethodCall, result: MethodChannel.Result) {
        if (pendingStartResult != null) {
            result.error("BUSY", "VPN is already starting", null)
            return
        }
        ...
        beginStartMonitoring(result)
        runCatching {
            startVpnService(xrayConfig, proxyOnly)
        }.onFailure {
            completeStartWithError("VPN_START_ERROR", "Failed to request VPN start: ${it.message}")
        }
    }
    ...
    private fun buildXrayConfig(call: MethodCall, config: String) = XrayConfig().apply {
        ...
        // Populate connected server endpoint used by VPN route exclusion.
    ...
    private fun extractServerAddress(config: XrayConfig) {
        runCatching {
            ...
            // vmess / vless
            val vnextServer = settings?.optJSONArray("vnext")?.optJSONObject(0)
            // trojan / shadowsocks / socks
            val directServer = settings?.optJSONArray("servers")?.optJSONObject(0)
            ...
            if (server != null) {
                ...
                config.CONNECTED_V2RAY_SERVER_PORT = "0"
            }
        }.onFailure {
    ...
    private fun handleStopVless(result: MethodChannel.Result) {
        completeStartWithError("VPN_START_CANCELLED", "VPN start cancelled by stop request")
        context?.let { ctx ->
            ...
            try {
                ...
                android.util.Log.d("DartV2ray", "STOP_SERVICE command sent via startService")
            } catch (e: Exception) {
                // Fallback to stopService if background restrictions apply (Android 12+)
                android.util.Log.w("DartV2ray", "Failed to startService with STOP command, falling back to stopService", e)
    ...
    private fun handleInitialize(call: MethodCall, result: MethodChannel.Result) {
        ...
        AppConfigs.ALLOW_VPN_FROM_SETTINGS = call.argument<Boolean>("allowVpnFromSettings") ?: true
        result.success(null)
    }

    private fun beginStartMonitoring(result: MethodChannel.Result) {
        pendingStartResult = result
        unregisterStartMonitor()

        startMonitorReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                when (state) {
                    ...
                    AppConfigs.V2RAY_STATES.V2RAY_AUTO_DISCONNECTED -> {
                        ...
                        completeStartWithError(
                            "VPN_START_FAILED",
                            errorDetails ?: "VPN disconnected while starting"
                        )
                    }
                    else -> {
                        // Ignore CONNECTING updates while we wait for final state.
                    }
                }
            }
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            context.registerReceiver(startMonitorReceiver, filter, Context.RECEIVER_EXPORTED)
        } else {
            context.registerReceiver(startMonitorReceiver, filter)
        }

        startTimeoutRunnable = Runnable {
            completeStartWithError("VPN_START_TIMEOUT", "Timed out waiting for VPN to connect")
        }.also { mainHandler.postDelayed(it, START_RESULT_TIMEOUT_MS) }
    }

    private fun unregisterStartMonitor() {
        startTimeoutRunnable?.let { mainHandler.removeCallbacks(it) }
        startTimeoutRunnable = null

        startMonitorReceiver?.let { receiver ->
            runCatching { context.unregisterReceiver(receiver) }
        }
        startMonitorReceiver = null
    }

    private fun completeStartSuccess() {
    ...
    private fun completeStartWithError(code: String, message: String) {
        val callback = pendingStartResult ?: return
        unregisterStartMonitor()
        pendingStartResult = null
    ...
    private fun emitCurrentStatusSnapshot() {
        val stateName = mapStateName(AppConfigs.V2RAY_STATE)
        val remaining = XrayCoreManager.getRemainingAutoDisconnectTime()
            .takeIf { it >= 0 }
            ?.toString()
        val transportMode = when (AppConfigs.V2RAY_CONNECTION_MODE) {
    ...
    companion object {
        private const val TAG = "DartV2rayPlugin"
        
        // Channel Names
        private const val METHOD_CHANNEL_NAME = "dart_v2ray"
        private const val EVENT_CHANNEL_NAME = "dart_v2ray/status"
        
        // Request Codes
        private const val REQUEST_CODE_VPN_PERMISSION = 24
        private const val REQUEST_CODE_POST_NOTIFICATIONS = 1
        private const val START_RESULT_TIMEOUT_MS = 15_000L
