package com.dart.v2ray.vpn.xray.service
...
open class XrayVPNService : VpnService() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val internalStart = intent.getBooleanExtra("INTERNAL_START", false)

        if (!AppConfigs.ALLOW_VPN_FROM_SETTINGS &&
            !internalStart &&
            (command == AppConfigs.V2RAY_SERVICE_COMMANDS.START_SERVICE || command == null)
        ) {
            Log.w(TAG, "Blocked external VPN start (settings/system) by configuration")
            stopAll()
    ...
    private fun configureRoutes(builder: Builder, config: XrayConfig) {
        ...
        if (serverIp.isNotEmpty() && !serverIp.contains(":")) {
            runCatching {
                ...
                bypassSubnets += excludeIp(serverIp).mapNotNull { parseCidrRoute(it) }
            }.onFailure {
        ...
        if (bypassSubnets.isNotEmpty()) {
            bypassSubnets.forEach { (address, prefix) ->
    ...
    private fun runTun2socks(config: XrayConfig) {
        ...
        tun2socksExecutable.setExecutable(true, false)
        val tun2socksPath = tun2socksExecutable.absolutePath
    ...
    private fun parseCidrRoute(route: String): Pair<String, Int>? {
        val cidr = route.trim()
        if (cidr.isEmpty()) return null

        val parts = cidr.split("/")
        if (parts.size != 2) {
            Log.w(TAG, "Ignoring invalid route (missing /): $route")
            return null
        }

        val address = parts[0].trim()
        val prefix = parts[1].trim().toIntOrNull()
        val maxPrefix = if (address.contains(":")) 128 else 32
        if (prefix == null || prefix !in 0..maxPrefix) {
            Log.w(TAG, "Ignoring invalid route prefix: $route")
            return null
        }
