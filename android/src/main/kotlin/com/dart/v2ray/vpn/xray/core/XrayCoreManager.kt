package com.dart.v2ray.vpn.xray.core
...
import com.dart.v2ray.vpn.xray.dto.XrayConfig
import com.dart.v2ray.vpn.xray.service.XrayVPNService
import com.dart.v2ray.vpn.xray.utils.AppConfigs
import com.dart.v2ray.vpn.xray.utils.Utilities
...
object XrayCoreManager {
    ...
    private var lastTrafficSampleTimeMs = 0L
    private var lastTotalUploadBytes = 0L
    private var lastTotalDownloadBytes = 0L
    private var cachedTraffic = longArrayOf(0L, 0L, 0L, 0L)
    ...
    fun startCore(context: Service, config: XrayConfig): Boolean {
        ...
        return runCatching {
            ...
            val xrayExecutable = findXrayExecutable(context)
            ...
            lastStartError = buildStartErrorMessage(context, it)
            Log.e(TAG, "Failed to start Xray Core: ${lastStartError}", it)
        }.getOrDefault(false)
    }
    ...
    private fun findXrayExecutable(context: Context): File {
        Utilities.resolveNativeExecutable(context, XRAY_EXECUTABLE_NAME)?.let { executable ->
            Log.d(TAG, "Resolved Xray executable: ${executable.absolutePath}")
            return executable
        }

        val nativeLibDir = File(context.applicationInfo.nativeLibraryDir)
        val deviceAbis = Build.SUPPORTED_ABIS.joinToString(", ")
        val nativeLibs = nativeLibDir.listFiles()
            ?.joinToString(", ") { it.name }
            ?.ifBlank { "<empty>" }
            ?: "<unreadable>"
        val sourceApk = context.applicationInfo.sourceDir ?: "<unknown>"
        val extractNativeLibs =
            (context.applicationInfo.flags and ApplicationInfo.FLAG_EXTRACT_NATIVE_LIBS) != 0
        val abiHint = if (Build.SUPPORTED_ABIS.none { it.contains("arm", ignoreCase = true) }) {
            "This plugin currently bundles only arm64-v8a and armeabi-v7a native binaries. " +
                "Use an ARM device/emulator or add x86/x86_64 builds for Xray and tun2socks."
        } else {
            "Make sure libxray.so is packaged for the current ABI and, if needed, set android:extractNativeLibs=\"true\"."
        }

        throw IllegalStateException(
            "Xray executable not found at ${File(nativeLibDir, XRAY_EXECUTABLE_NAME).absolutePath}. " +
                "deviceAbis=[$deviceAbis], nativeLibraryDir=${nativeLibDir.absolutePath}, " +
                "availableNativeLibs=[$nativeLibs], sourceApk=$sourceApk, extractNativeLibs=$extractNativeLibs. $abiHint"
        )
    }

    private fun buildStartErrorMessage(context: Context, error: Throwable): String {
        val message = error.message?.trim()
        if (!message.isNullOrEmpty()) return message

        val nativeDir = context.applicationInfo.nativeLibraryDir
        return "Failed to start Xray Core process (nativeLibraryDir=$nativeDir, " +
    ...
    private fun startTimer(context: Context) {
        ...
        lastTrafficSampleTimeMs = 0L
        lastTotalUploadBytes = 0L
        lastTotalDownloadBytes = 0L
        cachedTraffic = longArrayOf(0L, 0L, 0L, 0L)
        ...
        countDownTimer = object : CountDownTimer(Long.MAX_VALUE, TIMER_INTERVAL_MS) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                val traffic = sampleTrafficIfNeeded(context)
                broadcastConnectionStatus(context, traffic)
            }
    ...
    private fun broadcastConnectionStatus(context: Context, traffic: LongArray) {
    ...
    private fun sampleTrafficIfNeeded(context: Context): LongArray {
        ...
        if (connectionDurationSeconds == 1 || connectionDurationSeconds % TRAFFIC_SAMPLE_INTERVAL_SECONDS == 0) {
            cachedTraffic = getV2rayTraffic(context)
        }
    ...
    fun getV2rayTraffic(context: Context): LongArray {
        ...
        return runCatching {
            ...
            val totals = parseTrafficTotals(output)
            val now = System.currentTimeMillis()
            val totalUpload = totals[0]
            val totalDownload = totals[1]

            val uploadSpeed = if (lastTrafficSampleTimeMs > 0L) {
                val elapsedMs = (now - lastTrafficSampleTimeMs).coerceAtLeast(1L)
                ((totalUpload - lastTotalUploadBytes).coerceAtLeast(0L) * 1000L) / elapsedMs
            } else {
                0L
            }

            val downloadSpeed = if (lastTrafficSampleTimeMs > 0L) {
                val elapsedMs = (now - lastTrafficSampleTimeMs).coerceAtLeast(1L)
                ((totalDownload - lastTotalDownloadBytes).coerceAtLeast(0L) * 1000L) / elapsedMs
            } else {
                0L
            }

            lastTrafficSampleTimeMs = now
            lastTotalUploadBytes = totalUpload
            lastTotalDownloadBytes = totalDownload

            longArrayOf(uploadSpeed, downloadSpeed, totalUpload, totalDownload)
        }.onFailure {
            Log.e(TAG, "Failed to query traffic stats", it)
        }.getOrDefault(ZERO_TRAFFIC)
    }

    private fun parseTrafficTotals(jsonOutput: String): LongArray {
        if (jsonOutput.isEmpty()) return longArrayOf(0L, 0L)

        return runCatching {
            val json = JSONObject(jsonOutput)
            val stats = json.optJSONArray("stat") ?: return longArrayOf(0L, 0L)
            ...
            longArrayOf(uplink, downlink)
        }.getOrDefault(longArrayOf(0L, 0L))
    }
